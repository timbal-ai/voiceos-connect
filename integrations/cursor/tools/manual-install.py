#!/usr/bin/env python3
"""Registers the notch-coder integration in VoiceOS's config store — the exact
record IntegrationManager.install() writes (decoded from app main.js v0.1.21).
Run only while VoiceOS is closed."""
import json, hashlib, base64, re, shutil, os
...
DEFAULT_REPO = "/Users/dberges/Desktop/timbal-ai/voiceos-demo"
GITHUB_REPO = "https://github.com/timbal-ai/notch-coder-demo"

key = None
for line in open(os.path.join(DIR, ".env")):
    m = re.match(r"CURSOR_API_KEY=(.+)", line.strip())
    if m:
        key = m.group(1)
assert key, "no key in .env"
...
record = {
    ...
    "serverId": server_id,
    "configValues": {
        # plaintext is fine: encryptLegacySecretsAtRest encrypts at next boot
        "CURSOR_API_KEY": key,
...
shutil.copy(CONFIG, CONFIG + ".bak-notchcoder")
...
print("records now:", len(cfg["installedIntegrations"]))
